<template>
  <div class="clearfix selector">
    ...
    <div class="type-wrap" v-for="(attr, index) in attrsList" :key="attr.attrId">
      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(attrValue, index) in attr.attrValueList" :key="attrValue.index"
...
export default {
  ...
  methods: {
    ...
    trademarkHandler(trademark) {
      // 点击了品牌之后，还是需要整理参数，向服务器发起请求获取相应数据进行展示
      // 通过子 -> 父 组件间通信，实现子组件将点击的品牌的信息传递给父组件，父组件将信息整理到searchParams后向服务器发起请求
      // 子组件中触发自定义事件并传参
