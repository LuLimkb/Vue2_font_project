        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr, index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{ active: spuSaleAttrValue.isChecked == 1 }"
                  ...
                  @click="setActive(spuSaleAttrValue, spuSaleAttr.spuSaleAttrValueList)">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前路由跳转，是从A路由跳转到B路由，这里加入购物车进行路由跳转之前，发请求把购买的产品信息通过请求的形式通知服务器，服务器进行相应的存储 -->
                <a href="javascript:" @click="addToShopCat">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch('getGoodInfo', this.$route.params.goodId);
  },
  computed: {
  ...
  methods: {
    // 排他思想设置点击的属性高亮
    setActive(spuSaleAttrValue, arr) {
      arr.forEach(item => {
        item.isChecked = 0
      });
    ...
    async addToShopCat() {
      // 1.发请求，将产品加入到数据库
      // 2.判断加入购物车是否成功还是失败，并进行相应操纵
      try {
        await this.$store.dispatch('addUpdateShopCar', { skuId: this.$route.params.goodId, skuNum: this.skuNum });
        // 成功后进行路由跳转
        // 简单的数据skuNum使用query传递
        // 复杂的数据skuInfo【对象】使用会话存储sessionStorage
